...
class _AdminNewsPageState extends State<AdminNewsPage> {
  ...
  Future<void> _editNews(int id, String currentContent) async {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text("แก้ไขประกาศ", style: TextStyle(color: AppColors.brownDark, fontWeight: FontWeight.bold)),
        content: TextField(
          controller: editController, 
          maxLines: 3,
          decoration: const InputDecoration(
            focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: AppColors.brownDark)),
        ...
        actions: [
          ...
          ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: AppColors.brownDark),
            onPressed: () async {
              try {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("จัดการข่าวสาร (Admin)", style: TextStyle(color: Colors.white)),
        ...
        children: [
              ...
              children: [
                ...
                _isLoading
                    ...
                    : ElevatedButton.icon(
                        ...
                        label: const Text("ประกาศข่าว", style: TextStyle(color: Colors.white, fontSize: 16)),
                      ),
              ],
            ),
          ),
          ...
          const Padding(
            ...
            child: Align(
              ...
              child: Text(
                "รายการประกาศทั้งหมด",
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: AppColors.brownDark),
              ),
            ),
          ),
            ...
            child: FutureBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return const Center(child: CircularProgressIndicator(color: AppColors.brownDark));
                }
                if (snapshot.hasError) {
                  return Center(child: Text("เกิดข้อผิดพลาด: ${snapshot.error}"));
                }
                if (!snapshot.hasData || snapshot.data!.isEmpty) {
                  return const Center(child: Text("ยังไม่มีประกาศข่าว", style: TextStyle(color: Colors.grey)));
                }
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        leading: CircleAvatar(
                          backgroundColor: AppColors.brownLight.withOpacity(0.2),
                          child: const Icon(Icons.campaign, color: AppColors.brownDark),
                        ),
                        title: Text(
                          item['news'] ?? '',
                        ...
                        subtitle: Padding(
                          ...
                          child: Text("ID: ${item['id']}", style: const TextStyle(fontSize: 12, color: Colors.grey)),
                        ),
                        trailing: Row(
                          ...
                          children: [
                            IconButton(
                              ...
                              onPressed: () => _editNews(item['id'], item['news']),
                            ),
                            IconButton(
