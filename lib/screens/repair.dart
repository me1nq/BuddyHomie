...
class _RepairPageState extends State<RepairPage> {
  // Key สำหรับ Scaffold เพื่อให้ CustomHeader สั่งเปิด Drawer ได้
  ...
  Future<void> _submitRepair() async {
    if (_titleController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('กรุณาระบุหัวข้อปัญหา')),
      );
      return;
    }
    ...
    try {
      ...
      if (_imageFile != null) {
        // ตั้งชื่อไฟล์ด้วยเวลาปัจจุบัน (ชื่อจะได้ไม่ซ้ำ)
        final fileName = '${DateTime.now().millisecondsSinceEpoch}.jpg';
        
        // สั่งอัปโหลดไฟล์ขึ้น Storage Bucket ชื่อ 'repair_images'
        await _supabase.storage.from('repair_images').upload(fileName, _imageFile!);
        
        // ดึง URL ของรูปมาเก็บไว้ เพื่อบันทึกลง Database
        imagePath = _supabase.storage.from('repair_images').getPublicUrl(fileName);
      }

      // 2. Insert ลง Table 'repair'
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      endDrawer: const SideMenuDrawer(), // ใส่ Drawer ไว้ที่นี่เพื่อให้ Header เรียกใช้ได้
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildTextField(_titleController, 'เช่น หลอดไฟดับ, ท่อน้ำรั่ว', height: 50),
                    ...
                    child: Container(
                      ...
                      child: _imageFile == null
                          ? Column(
                              ...
                              children: const [
                                Icon(Icons.add_a_photo, size: 40, color: AppColors.brownLight),
                                SizedBox(height: 8),
                                Text(
                    ...
                    builder: (context, snapshot) {
                      ...
                      if (!snapshot.hasData || snapshot.data!.isEmpty) {
                        return Container(
                          ...
                          child: const Text('ยังไม่มีประวัติการแจ้งซ่อม', style: TextStyle(color: Colors.grey)),
                        );
                      }
  ...
  Widget _buildLabel(String text) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: AppColors.brownLight, // สีน้ำตาลอ่อน
